...
# Get the HTML set up
Create your HTML elements and declare the doctype like so:
...
In your head tag add the following: <meta description="hav some description for your page">
Quick rundown of vocabulary "meta" is the tag (like html, head, body), description is an attribute (which we'll see more instances of later)
Some tags have an opening and a closing and others self-close. A self-closing tag is usually one that doesn't contain anything that's meant to be displayed.

Now add <title></title> under the meta-description tag we've just put into our code. Within the title tag, put the title of the page "CV - YOUR NAME".
...
Opening this up will show you a blank page, but the "title" tag means that the browser knows how to name the page (you can see "CV - YOUR NAME" on the tab in which its open).
[IMAGE SCREEN 1]
...
[IMAGE WITH HIGHLIGHTED HEADER SECTION]

This is the header of the CV right? Well that's the name of the tag: `header`. Inside our header, we have headings of different sizes. The heading elements are `h1`, `h2`, `h3`, `h4` and so on.

Add the following code to your `cv.html` file.
```html
<header>
  <h1>Hugo Di Francesco</h1>
  <h2>Chief of head officer</h2>
</header>
```
Go to your browser and refresh the page. Ok it's not that pretty but hey, we're getting content up right?

Next we're going to tackle that section on the left, the one that has your contact details, skills, languages and so on. Again we're going to use a fancy HTML5 tag called `aside`. You can now call yourself a HTML5 ninja (disclosure I have never used this tag, ever because you can replicate its behaviour with generic elements but more on that later). Your body tag should look something like this:
```html
<body>
  <header>
    <h1>Hugo Di Francesco</h1>
    <h2>Chief Head Officer</h2>
  </header>
  <aside>
...
```html
<h3>Contact</h3>
<p><p><a href="http://twitter.com/hugo__df">@hugo__df</a></p>
...
```
Our next line is more interesting. In the `href`, we're stating `mailto`, this is a cool trick that allows you to tell the browser "hey, you should open an email browser with `hdifrancesco@gmail.com` as the recipient". Which, when you click it, is exactly what it does. You can look up more tricks like "telephone link" (which does something similar to what `mailto` does but for a phone).

What doe the `p`tags do? Well remove them and see. The `p` or paragraph tag tells the browser "this is a paragraph" so there's a line break after it. In HTML terms, `p` is displayed as a `block`, whereas `a`'s are displayed `inline`. Other elements that display as `block` (that we've seen) are `div`, `aside` and the `h_` (heading) family.
...
## The meat and potatoes of the CV
...
`class` is an attribute, because we're using `div` which is a generic element, we want to give that section a bit more meaning by calling it class (also we're going to need this to style it with CSS later). Apart from that (being used for styling and scripting), it doesn't do nothing functionally.

We're going to add sections/entries to your CV, an entry looks something like this:
```html
<div class="section">
  <h3>Education</h3>
    <div class="entry">
...
Why is there a `.` before `cv-container`? That's our way of telling the browser we're selecting a class. We explained selectors above and now we're applying this, `.cv-container` is a selector that selects all elements that have `class="cv-container"`. If we have selector `#menu-toggle` it would select the first element with `id = "menu-toggle"`.
...
# Lines away: deploying to GitHub Pages

Let's deploy (put this whole thing online).

What we want to do is create a git repo from the work we just did, push it up to github and then create a `gh-pages` branch so that we can see it online.
